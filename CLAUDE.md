...
- **Calendar dragging uses SwiftUI gestures, not system drag and drop.** Chips and event blocks get `.calendarDragSource`, and places that accept drops get `.calendarDropZone`, which reports its frame in the shared `CalendarDragState.space` coordinate space. `CalendarDrop.target` (in `Model/`, tested) picks the target under the pointer, so the calendar can preview the landing spot live. A drop animates the document change with `withAnimation`. Double-clicks on the calendar background check `NSApp.currentEvent?.clickCount` in a single tap, because an ancestor's `onTapGesture(count: 2)` makes chip clicks wait out the double-click interval before their popover opens. On the Week view's hour grid, the details popover is attached to the item's slot in `DayTimeline`, not to the `TaskChip`/`EventBlock` inside it, so switching between task and event swaps the view without closing the popover.
